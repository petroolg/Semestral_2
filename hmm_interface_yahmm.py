...
import numpy as np
from interface_yahmm import Adapter
...
def backward1(next_b, next_e, hmm):
    ...
    for state_curr in next_b:
        for state_next in next_b:
            cur_b[state_curr] = cur_b[state_curr] + hmm.pe(state_next, next_e) * hmm.pt(state_curr, state_next) * \
                                                    next_b[state_next]
    return cur_b
...
def forwardbackward(priors, e_seq, hmm):
    ...
    a = Adapter(hmm, priors, e_seq)
    se = a.forwardbackward()
    return se
